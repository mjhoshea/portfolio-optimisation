...
class HistoricalHarnessVec:
    ...
    def historical_test(self):
        ...
        policy_ans = self._policy.calc_an()
        policy_ws = self._policy.act(policy_ans, n_samples)

        # calculate the returns from the policy and the baseline
        eq_ws__rs = np.sum(test_data * eq_ws, axis=1)
        policy_ws_rs = np.sum(test_data * policy_ws, axis=1)
